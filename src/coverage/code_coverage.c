...
*/
	for (i = 0; i < last_line; i++) {
		...
		if (i < file->runtime.hit_lines->size) {
...
void xdebug_coverage_count_line_if_branch_check_active(function_stack_entry *fse)
{
	if (XG_COV(code_coverage_active) && XG_COV(code_coverage_branch_check)) {
		xdebug_count_line(fse->filename, fse->lineno);
	}
}
